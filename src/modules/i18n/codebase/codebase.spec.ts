...
describe('Codebase', () => {
	...
	describe('getTranslationProgress()', () => {
		setup.it('should return a progress report for each language', async () => {
			translationFiles.getAllTranslationsFromAllLanguages.resolves({
				en: [],
				el: [],
			})

			const reports = await codebase.getTranslationProgress()

			expect(reports.length).to.equal(2)
		})

		setup.it('should return an accurate progress report', async () => {
			translationFiles.getAllTranslationsFromAllLanguages.resolves({
				en: [new Translation('settings', 'Settings'), new Translation('user_male', 'User'), new Translation('user_female', 'User')],
				el: [],
			})

			const [enReport, elReport] = await codebase.getTranslationProgress()

			expect(enReport.language).to.equal('en')
			expect(enReport.translatedStrings).to.equal(3)
			expect(enReport.percentageOfStringsTranslated).to.equal(1)

			expect(elReport.language).to.equal('el')
			expect(elReport.translatedStrings).to.equal(0)
			expect(elReport.percentageOfStringsTranslated).to.equal(0)
		})
	})
})
